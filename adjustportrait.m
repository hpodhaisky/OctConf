...
switch flag
 case 'start' % Initialize GUI
  ...
  ud.button(5) = uicontrol(f,'Position',[.55 .5 .2 .05],...
    'String','Next please!','FontSize',20,'Callback',...
	'adjustportraits(''nextplease'')');
  ud.currfolder = ['.',filesep,'neueBilderblank']; % folder files
  ud.all = dir(ud.currfolder); ud.nall = size(ud.all, 1); ud.i = 2;
  ud.verz = [ud.currfolder,filesep,'tmp',filesep]; % folder new files 
